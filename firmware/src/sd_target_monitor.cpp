...
CdcSdBackend backend;
picosd::protocol::SdCardModel card{picosd::protocol::SdCardType::Sdhc, backend};
...
void update_chip_select_state() {
    ...
    if (chip_select_was_asserted && !chip_select_asserted) {
        ...
        transaction_timed_out = false;
        if (trace_enabled)
            std::printf("TRACE_TARGET_CS released\n");
    }
    chip_select_was_asserted = chip_select_asserted;
}

void check_transaction_timeout() {
    if (!chip_select_was_asserted || transaction_timed_out)
        return;
    if (time_us_64() - last_activity_us < kTransactionTimeoutUs)
        return;
    worker.transaction_timed_out();
    cancel_spi_transmit();
    transaction_timed_out = true;
    if (trace_enabled)
        std::printf("TRACE_TARGET_TIMEOUT\n");
}
} // namespace

void initialize_sd_target_monitor() {
    backend.refresh_media();
    card.refresh_capacity();
}
...
bool poll_sd_target_monitor() {
    ...
    for (unsigned int count = 0; count < kMaximumBytesPerPoll; ++count) {
        std::uint8_t byte = 0;
        if (!try_read_spi_capture_byte(byte))
            break;
...
void print_sd_target_monitor_counters() {
    ...
    std::printf("TARGET_COUNTERS rx=%zu tx=%zu rx_overflow=%zu tx_overflow=%zu tx_underrun=%zu "
