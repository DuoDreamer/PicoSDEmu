...
int main() {
    ...
    expect(card.reset() == SdCardStateError::None && card.state() == SdCardState::Idle,
           "reset returns idle");
    card.fault();
    ...
    block[0] = 0xa5U;
    expect(backend.write(1, block) == BlockOperationResult::Complete, "in-range write succeeds");
    SdBlock readback{};
    expect(backend.read(1, readback) == BlockOperationResult::Complete && readback[0] == 0xa5U,
