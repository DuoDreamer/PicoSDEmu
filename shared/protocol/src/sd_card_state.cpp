...
SdCardState SdCardStateMachine::state() const {
    return state_;
}

SdCardStateError SdCardStateMachine::finish_power_up() {
    if (state_ != SdCardState::PowerUp)
        return SdCardStateError::InvalidTransition;
    state_ = SdCardState::Idle;
    return SdCardStateError::None;
}

SdCardStateError SdCardStateMachine::reset() {
    if (state_ == SdCardState::PowerUp || state_ == SdCardState::Fault)
        return SdCardStateError::InvalidTransition;
    state_ = SdCardState::Idle;
    return SdCardStateError::None;
}
...
SdCardStateError SdCardStateMachine::begin_transfer() {
    if (state_ != SdCardState::Ready && state_ != SdCardState::Transfer)
        return SdCardStateError::InvalidTransition;
...
SdCardStateError SdCardStateMachine::begin_receiving_data() {
    if (state_ != SdCardState::Transfer)
        return SdCardStateError::InvalidTransition;
...
SdCardStateError SdCardStateMachine::begin_busy() {
    if (state_ != SdCardState::ReceivingData)
        return SdCardStateError::InvalidTransition;
...
void SdCardStateMachine::fault() {
    state_ = SdCardState::Fault;
}

RamBlockBackend::RamBlockBackend(std::size_t block_count)
    : block_count_(block_count <= kRamBackendMaximumBlocks ? block_count : 0) {}

std::size_t RamBlockBackend::block_count() const {
    return block_count_;
}

BlockOperationResult RamBlockBackend::read(std::size_t lba, SdBlock &output) const {
    if (lba >= block_count_)
...
BlockOperationResult RamBlockBackend::write(std::size_t lba, const SdBlock &input) {
    if (lba >= block_count_)
        return BlockOperationResult::Failed;
    blocks_[lba] = input;
    return BlockOperationResult::Complete;
